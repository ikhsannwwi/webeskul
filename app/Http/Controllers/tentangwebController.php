...
class tentangwebController extends Controller
{
    ...
    public function insertdatatentangweb(Request $request){
        $data = tentangweb::create($request->all());
        if($request->hasfile('logoitc')){
            $request->file('logoitc')->move('logotentangweb/', $request->file('logoitc')->getClientOriginalName());
            $data->logoitc = $request->file('logoitc')->getClientOriginalName();
            $data->save();
        }
